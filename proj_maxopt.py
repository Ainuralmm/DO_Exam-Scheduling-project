...
u = model.addVar(vtype=GRB.INTEGER, name='u') # Min distance between 2 conflicting exams
...
for (e1,e2) in conflicting_exams.keys():
    for t1 in time_slots:
        for t2 in time_slots:
            if t1 < t2:
                model.addConstr(u <= (x[t1,e1]+x[t2,e2]-1)*(t2-t1-len(time_slots))+len(time_slots))
                model.addConstr(u <= (x[t2,e1]+x[t1,e2]-1)*(t2-t1-len(time_slots))+len(time_slots))
